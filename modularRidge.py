...
class LearnHam:
    ...
    def ridgereg(self,lam):
        self.ridgeX = 0.5*self.hess + lam*np.eye(self.hess.shape[0])
        self.ridgeY = -0.5*self.grad
        theta,_,_,_ = sl.lstsq(self.ridgeX, self.ridgeY, lapack_driver='gelsy')
        loss = self.newloss(theta)
        return loss,theta 
...
#         self.trainloss = self.newloss(self.theta)
        lamrange = np.logspace(-8,0,50)
        pool = multiprocessing.Pool()
        loss = pool.map(self.ridgereg, lamrange)
        pool.close()
        pool.join()
        self.trainloss,index = min((value[0],index) for (index,value) in enumerate(loss))
        self.theta = loss[index][1]       
    ...
    def propagate(self, rhsfunc, initcond, intpts=4000, mytol=1e-12):
        self.intpts = intpts
...
#     print('******')
    ...
    mlham.trainmodel()
#     mlham.loadtheta()
#     mlham.ridgereg(np.logspace(-10,0,30))
