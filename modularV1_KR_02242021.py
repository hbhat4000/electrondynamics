...
class LearnHam:
    ...
    # class initializer
    def __init__(self, mol, outpath, basis, initmethod, tdmethod):
        
        # store the short string that indicates which molecule and basis set we're working on
        self.mol = mol
        self.basis = basis

        self.initmethod = initmethod
        self.tdmethod = tdmethod
        
        # field sign correction
        if self.tdmethod == 'rt-tdexx':
            if self.mol == 'h2':
                self.fieldsign = -1
            else:
                self.fieldsign = 1
    ...
    def load(self,inpath,pert):
        self.perttrain = pert
        # store the path to input files, i.e., training data, auxiliary matrices, etc
        inpath = inpath
        rawden = onp.load(inpath + 'td_dens_re+im_'+tdmethod+'_'+pert+'_s0_'+mol+'_'+basis+'.npz',allow_pickle=True)
        overlap = onp.load(inpath + 'ke+en+overlap+ee_twoe+dip_'+initmethod+'_'+pert+'_s0_'+mol+'_'+basis+'.npz',allow_pickle=True)
        try:
            fielddata = onp.load(inpath + 'td_efield_'+tdmethod+'_'+pert+'_s0_'+mol+'_'+basis+'.npz',allow_pickle=True)
            self.eftrainx = fielddata['td_efield_data'][0,:]
            self.eftrainy = fielddata['td_efield_data'][1,:]
            self.eftrainz = fielddata['td_efield_data'][2,:]
        except FileNotFoundError:
            try:
                fielddata = onp.load(inpath + 'td_efield+dipole_'+tdmethod+'_'+pert+'_s0_'+mol+'_'+basis+'.npz',allow_pickle=True)
                self.eftrainx = fielddata['td_efield_data'][0,:]
                self.eftrainy = fielddata['td_efield_data'][1,:]
                self.eftrainz = fielddata['td_efield_data'][2,:]
            except FileNotFoundError:
                self.eftrainx = onp.zeros(shape=(rawden['td_dens_re_data'].shape[0]))
                self.eftrainy = onp.zeros(shape=(rawden['td_dens_re_data'].shape[0]))
                self.eftrainz = onp.zeros(shape=(rawden['td_dens_re_data'].shape[0]))
        ...
        den = realpt + 1j*rawden['td_dens_im_data']
        ...
        self.eftrain = onp.zeros(shape=(self.eftrainx.shape[0],self.drc,self.drc))
        for i in range(self.eftrain.shape[0]):
            self.eftrain[i,:,:] = self.eftrainx[i]*self.didat[0]
            self.eftrain[i,:,:] += self.eftrainy[i]*self.didat[1]
            self.eftrain[i,:,:] += self.eftrainz[i]*self.didat[2]
        ...
        eftraincommute = onp.zeros(shape=(self.eftrain.shape[0],self.drc,self.drc), dtype=onp.complex128)
        for i in range(self.eftrain.shape[0]):
            dv = onp.diag(self.sevals**(0.5)) @ self.sevecs.T @ self.eftrain[i,:,:] @ self.sevecs @ onp.diag(self.sevals**(0.5))
            p = self.denMO[i,:,:]
            eftraincommute[i,:,:] = (dv @ p - p @ dv)/(1J)
        self.eftraincommuteMO = np.asarray(eftraincommute)
        self.eftraincommuteMOflat = self.eftraincommuteMO.reshape((-1,self.drc**2))
    ...
    def loadfield(self,inpath,pert):
        self.perttest = pert
        try:
            fielddata = onp.load(inpath + 'td_efield_'+tdmethod+'_'+pert+'_s0_'+mol+'_'+basis+'.npz',allow_pickle=True)
            self.eftestx = fielddata['td_efield_data'][0,:]
            self.eftesty = fielddata['td_efield_data'][1,:]
            self.eftestz = fielddata['td_efield_data'][2,:]
        except FileNotFoundError:
            try:
                fielddata = onp.load(inpath + 'td_efield+dipole_'+tdmethod+'_'+pert+'_s0_'+mol+'_'+basis+'.npz',allow_pickle=True)
                self.eftestx = fielddata['td_efield_data'][0,:]
                ...
                self.eftestx = onp.zeros(shape=(rawden['td_dens_re_data'].shape[0]))
                self.eftesty = onp.zeros(shape=(rawden['td_dens_re_data'].shape[0]))
                self.eftestz = onp.zeros(shape=(rawden['td_dens_re_data'].shape[0]))
        # self.efdat = fielddata['td_efield_data']

        self.eftest = onp.zeros(shape=(self.eftestx.shape[0],self.drc,self.drc))
        for i in range(self.eftestx.shape[0]):
            self.eftest[i,:,:] = self.eftestx[i]*self.didat[0]
            self.eftest[i,:,:] += self.eftesty[i]*self.didat[1]
            self.eftest[i,:,:] += self.eftestz[i]*self.didat[2]

        fielddens = onp.load(inpath + 'td_dens_re+im_'+tdmethod+'_'+pert+'_s0_'+mol+'_'+basis+'.npz',allow_pickle=True)
        ...
        if self.tdmethod == 'rt-tdexx':
            ...
            self.fielddenMOflat = fielddenMOflat
    ...
    def trainsplit(self, ntrain=1000, tt=4000):
        ...
        self.hpcommute_train_loss = self.hpcommute_train[1:(self.ntrain-1),:]
    ...
    def myloss(self, theta):
        ...
        hr = len(self.hamreals)
        pred_real = pred[:,:hr]
        pred_imag = pred[:,hr:]
        predfull_real = self.hpcommute_train_loss[:,:self.drc**2].reshape((-1,self.drc,self.drc))
        predfull_imag = self.hpcommute_train_loss[:,self.drc**2:].reshape((-1,self.drc,self.drc))
        for ij in self.hamreals:
            x = predfull_real[:,ij[0],ij[1]]
            predfull_real[:,ij[0],ij[1]] = x + pred_real[:,self.hamreals[ij]]
            predfull_real[:,ij[1],ij[0]] = x + pred_real[:,self.hamreals[ij]]
        for ij in self.hamimags:
            x = predfull_imag[:,ij[0],ij[1]]
            predfull_imag[:,ij[0],ij[1]] = x + pred_imag[:,self.hamimags[ij]]
            predfull_imag[:,ij[1],ij[0]] = x - pred_imag[:,self.hamimags[ij]]
        predfull_real = predfull_real.reshape((-1,self.drc**2))
        predfull_imag = predfull_imag.reshape((-1,self.drc**2))
        predfull = np.hstack([predfull_real, predfull_imag])
        pr = len(self.nzreals)
        xdot_real = self.xdot[:,:pr]
        xdot_imag = self.xdot[:,pr:]
        xdotfull_real = np.zeros(shape=(predfull.shape[0],self.drc,self.drc))
        xdotfull_imag = np.zeros(shape=(xdotfull_real.shape))
        for ij in self.nzreals:
            xdotfull_real[:,ij[0],ij[1]] = xdot_real[:,self.nzreals[ij]]
            xdotfull_real[:,ij[1],ij[0]] = xdot_real[:,self.nzreals[ij]]
        for ij in self.nzimags:
            xdotfull_imag[:,ij[0],ij[1]] = xdot_imag[:,self.nzimags[ij]]
            xdotfull_imag[:,ij[1],ij[0]] = -1*xdot_imag[:,self.nzimags[ij]]
        xdotfull_real = xdotfull_real.reshape((-1,self.drc**2))
        xdotfull_imag = xdotfull_imag.reshape((-1,self.drc**2))
        xdotfull = np.hstack([xdotfull_real, xdotfull_imag])
        loss = np.sum(np.square(xdotfull - predfull))
        return loss
    ...
    # Karnamohit's function (July 1 version) has been commented out.
    # Dr. Bhat's eeten1/2 version is faster (400x for paranitroaniline, as of 02/22/2021).
    ...
    def get_ee_onee_AO(self, dens, exchange=True):
        ...
        e = True
        eeten1 = self.eeten.reshape((self.drc, self.drc, self.drc**2))
        if (dens.shape[0] == self.eeten.shape[0]):
            nbas = dens.shape[0]
            vee_data = onp.zeros((nbas, nbas), dtype=onp.complex128)
            e = False
            if (exchange == True):
                eeten2 = np.swapaxes(self.eeten, 1, 2).reshape((self.drc, self.drc, self.drc**2))
                for u in range(nbas):
                    for v in range(u,nbas):
                        ...
                        vee_data[u,v] += 2*eeten1[u,v,:] @ dens.reshape((-1)) 
                        vee_data[u,v] -= eeten2[u,v,:] @ dens.reshape((-1))
    ...
    def EXhamrhs(self, t, pin):
        if self.tdmethod == 'rt-tdexx' and self.perttrain == 'delta':
            ez = 0
            hfieldAO = onp.array(ez*self.didat[2], dtype=onp.complex128)
        else:
            Nt = int(t/self.dt)
            ef = self.eftrain[Nt,:,:]
            hfieldAO = onp.array(ef, dtype=onp.complex128)
    ...
    def graphcomparetraj(self, traj1, traj2, groundtruth, myfigsize=(8,16), includeField=False, fname='prop.pdf', mytitle=None):
        ...
        if mytitle == None:
            if self.tdmethod == 'rt=tdexx':
                ...
                mytitle = 'TDCI script (black), exact-H (blue), and ML-H (red) propagation results'
...
if __name__ == '__main__':
    ...
    initmethod = 'hf'
    tdmethod = 'tdcasscf22'
    if tdmethod == 'rt-tdexx':
        mlham = LearnHam(mol,'./'+mol+'LINEAR/',basis,initmethod,tdmethod)
    elif 'tdcasscf' in tdmethod:
        import os; import shutil; import time
        dir_data = os.getcwd() + '/' # get the path of the current directory
        dir_data += 'TDCI/'+mol+'_'+tdmethod+'_'+'LINEAR/'
        ...
        try:
            ...
            if (os.path.isdir(dir_path) == True):
                ...
                time.sleep(2)
                print('  pre-exisiting directories deleted.')
        ...
        mlham = LearnHam(mol,'./TDCI/'+mol+'_'+tdmethod+'_'+'LINEAR/',basis,initmethod,tdmethod)
    mlham.load('./data/','delta')
    mlham.loadfield('./data/','ndlaser1cyc')
    mlham.trainsplit()
    mlham.buildmodel()
