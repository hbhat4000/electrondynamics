...
auxdata = onp.load('./ke+en+overlap+ee_twoe+dip_hf_ndlaser1cyc_s0_'+mol+'_6-31g.npz', allow_pickle=True)
...
fielddata = onp.load('./td_efield+dipole_rt-tdexx_ndlaser1cycs0_'+mol+'_6-31g.npz')
...
if mol == 'heh+':
    didat = [onp.zeros((4,4)), onp.zeros((4,4)), onp.zeros((4,4))]
    didat[2] = onp.array([[-0.729434,-0.480181,0.0664905,-0.162918],[-0.480181,-0.729434,0.192415,0.1620832],[0.0664905,0.192415,0.729434,0.462569],[-0.162918,0.162083,0.462569,0.729434]])

elif mol == 'h2':
    didat = [onp.zeros((4,4)), onp.zeros((4,4)), onp.zeros((4,4))]
    didat[2] = onp.array([[-0.6991990,0.,0.,0.],[-0.460277,-0.699199,0.,0.],[0.,0.224096,0.699199,0.],[-0.224096,0.000000,0.460277,0.699199]])
...
print(onp.linalg.norm( EXsol.y.T - denMOflat[offset:intpts,:] ))

# error between propagating machine learned Hamiltonian and Gaussian data
print(onp.linalg.norm( MLsol.y.T - denMOflat[offset:intpts,:] ))
...
axs = fig.subplots(d)
fig.suptitle('True (blue) and propagated-from-exact-H (red) density elements', y=0.9)

ctr = 0
for ij in nzreals:
    axs[ctr].plot(onp.real(EXsol.y.T.reshape((-1,drc,drc))[:,ij[0],ij[1]]), 'b-')
    axs[ctr].plot(onp.real(denMO[offset:intpts,ij[0],ij[1]]), 'r-')
    ctr += 1
...
fig.savefig(savepath + 'EXprop.pdf')
plt.close()

fig = plt.figure(figsize=((8,16)))
axs = fig.subplots(d)
fig.suptitle('True (blue) and propagated-from-ML-H (red) density elements', y=0.9)

ctr = 0
for ij in nzreals:
    axs[ctr].plot(onp.real(MLsol.y.T.reshape((-1,drc,drc))[:,ij[0],ij[1]]), 'b-')
    axs[ctr].plot(onp.real(denMO[offset:intpts,ij[0],ij[1]]), 'r-')
    ctr += 1

for ij in nzimags:
    axs[ctr].plot(onp.imag(MLsol.y.T.reshape((-1,drc,drc))[:,ij[0],ij[1]]), 'b-')
    axs[ctr].plot(onp.imag(denMO[offset:intpts,ij[0],ij[1]]), 'r-')
    ctr += 1

fig.savefig(savepath + 'MLprop.pdf')
plt.close()

fielddens = onp.load('./td_dens_re+im_rt-tdexx_ndlaser1cycs0_'+mol+'_6-31g.npz',allow_pickle=True)
...
print(np.mean(np.square(np.abs(EXsolwf.y.T - fielddenMOnodupflat[offset:intpts,:]))))

MLsolwf = si.solve_ivp(MLhamwfrhs, onp.array([0, tvec[-1]]), initcond, t_eval = tvec, rtol=1e-12, atol=1e-12)
print(np.mean(np.square(np.abs(MLsolwf.y.T - fielddenMOnodupflat[offset:intpts,:]))))
...
for ij in nzreals:
    sol1 = np.real(MLsolwf.y.T.reshape((-1,drc,drc))[:,ij[0],ij[1]])
    sol2 = np.real(fielddenMOnodup[offset:intpts,ij[0],ij[1]])
    plt.plot(np.abs(sol1-sol2))

for ij in nzimags:
    sol1 = np.imag(MLsolwf.y.T.reshape((-1,drc,drc))[:,ij[0],ij[1]])
    sol2 = np.imag(fielddenMOnodup[offset:intpts,ij[0],ij[1]])
    plt.plot(np.abs(sol1-sol2))
...
fig = plt.figure(figsize=((8,16)))
axs = fig.subplots(d)
fig.suptitle('With Field: True (blue) and propagated-from-exact-H (red) density elements', y=0.9)

ctr = 0
for ij in nzreals:
    axs[ctr].plot(onp.real(EXsolwf.y.T.reshape((-1,drc,drc))[:,ij[0],ij[1]]), 'b-')
    axs[ctr].plot(onp.real(fielddenMOnodup[offset:intpts,ij[0],ij[1]]), 'r-')
    ctr += 1

for ij in nzimags:
    axs[ctr].plot(onp.imag(EXsolwf.y.T.reshape((-1,drc,drc))[:,ij[0],ij[1]]), 'b-')
    axs[ctr].plot(onp.imag(fielddenMOnodup[offset:intpts,ij[0],ij[1]]), 'r-')
    ctr += 1

fig.savefig(savepath + 'EXWFprop.pdf')
plt.close()

fig = plt.figure(figsize=((8,16)))
axs = fig.subplots(d)
fig.suptitle('With Field: True (blue) and propagated-from-ML-H (red) density elements', y=0.9)

ctr = 0
for ij in nzreals:
    axs[ctr].plot(onp.real(MLsolwf.y.T.reshape((-1,drc,drc))[:,ij[0],ij[1]]), 'b-')
    axs[ctr].plot(onp.real(fielddenMOnodup[offset:intpts,ij[0],ij[1]]), 'r-')
    ctr += 1

for ij in nzimags:
    axs[ctr].plot(onp.imag(MLsolwf.y.T.reshape((-1,drc,drc))[:,ij[0],ij[1]]), 'b-')
    axs[ctr].plot(onp.imag(fielddenMOnodup[offset:intpts,ij[0],ij[1]]), 'r-')
    ctr += 1

fig.savefig(savepath + 'MLWFprop.pdf')
plt.close()
