...
class LearnHam:
    ...
    def load(self,inpath):
        ...
        # delete every 100th time-step from Gaussian data (MMUT restart)
    ...
    def loadfield(self,inpath):
        fielddata = np.load(inpath + 'td_efield+dipole_rt-tdexx_ndlaser1cyc_s0_'+self.mol+'_'+self.basis+'.npz')
        self.efdat = fielddata['td_efield_data']
        fielddens = np.load(inpath + 'td_dens_re+im_rt-tdexx_ndlaser1cyc_s0_'+self.mol+'_'+self.basis+'.npz',allow_pickle=True)
    ...
    def buildmodel(self, maxdeg=2):
        ...
        for j in range(1,self.maxdeg+1):
            ...
            # self.nump += self.ndof**np.minimum(2,j)
        # self.nump = self.ndof
        # if self.maxdeg > 1:
        #     for j in range(2,self.maxdeg+1):
        #         self.nump += self.ndof**2
